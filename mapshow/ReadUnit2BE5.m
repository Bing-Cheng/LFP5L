...
BSBE123(1,2) = mean((BERc1 + BERr1)/2);
BSBE123(2,2) = mean((BEWc1 + BEWr1)/2);
BSBE123(1,3) = mean((BERc3 + BERr3)/2);
BSBE123(2,3) = mean((BEWc3 + BEWr3)/2);
% [hr,pr,ci,stats] = ttest(BERc2+BERr2,BERc1+BERr1)
         h = figure; hold on;
         BSBE123=BSBE123/scaleDown;

               hR = plot([2:4],BSBE123(1,:),'-ro');
        hW =plot([2:4],BSBE123(2,:),'-bo');
         xlim([1 5]);
        legend([hR,hW],'SL','FD');
        set(gca,'XTick',[1:5])
     set(gca,'XTickLabel',{'' 'BS' 'BE' 'AE' ''});   
         titleName = ['Theta power comparison of BS AE and BE '];
 %   title(titleName,'FontSize',12);
    saveas(h, ['D:\bcheng\work\paper\my\paper2_LFP\fig\' titleName],'png');
    
        BSBE123a = zeros(4,2);

BSBE123a(1,1) = mean(BERc1);
BSBE123a(2,1) = mean(BERr1);
BSBE123a(3,1) = mean(BEWc1);
BSBE123a(4,1) = mean(BEWr1);
BSBE123a(1,2) = mean(BERc3);
BSBE123a(2,2) = mean(BERr3);
BSBE123a(3,2) = mean(BEWc3);
BSBE123a(4,2) = mean(BEWr3);

% BE = [mean([BERc1',BERr1'],2),mean([BEWc1',BEWr1'],2)];
% AE = [mean([BERc2',BERr2'],2),mean([BEWc2',BEWr2'],2)];
% [hr,pr,ci,stats] = ttest(BERc2+BERr2,BERc1+BERr1)
         h = figure; hold on;
         BSBE123a = BSBE123a/scaleDown;

        hRc= plot([2 3],BSBE123a(1,:),'-bo');
        hRr=plot([2 3],BSBE123a(2,:),'-ro');
        hWc= plot([2 3],BSBE123a(3,:),'-go');
        hWr = plot([2 3],BSBE123a(4,:),'-mo');
        xlim([1 4]);
        legend([hRc,hRr,hWc,hWr],'SL-AF','SL-AS','FD-AF','FD-AS');
        set(gca,'XTick',[1:4])
     set(gca,'XTickLabel',{''  'BE' 'AE' ''});   
         titleName = ['Theta power comparison of C and R for AE and BE '];
 %   title(titleName,'FontSize',12);
    saveas(h, ['D:\bcheng\work\paper\my\paper2_LFP\fig\' titleName],'jpg'); 
 Z = zeros(ratNumber*2,2);
Z(2:2:ratNumber*2,:) = [BERc1' BERc2'];%front BE, back AE, left AF, right AS
Z(1:2:ratNumber*2-1,:) = [BERr1' BERr2'];
%Z= Z2';
%ZW=Z;
  h = figure;
Z=Z/100000;
...
set(gca,'YTickLabel',{'AF '; '   AS'},'FontSize',5);  %# Modify the y axis tick labels 
set(gca,'XTickLabel',{'BE' 'AE'});  %# Modify the y axis tick labels x1:learned; x2:learning
set(gca,'ZTickLabel',{' '});
...
hc=colorbar('location','eastoutside','position',[pos(1)+pos(3)+0.03 pos(2) 0.03 pos(4)],'FontSize',5);
    ...
    titleName = ['Theta power comparison of AE and BE windows for AF and AS groups'];
   % title(titleName,'FontSize',12);
    saveas(h, ['D:\bcheng\work\paper\my\paper2_LFP\fig\' titleName],'jpg');
%     aZ = [Z(:,1);Z(:,2)]';
%     g0 = ['AF';'AS'];
%     g1 = repmat(g0,20,1);
%     g2 = [repmat('BE',20,1);repmat('AE',20,1)];
%     group = {g1;g2};
%     p2 = anovan(aZ,group);
%     tmp =1;       
        
     [hr,pr,ci,stats] = ttest(BERr1,BERr2)
     [hw,pc,ci,stats] = ttest(BERc1,BERc2)
     [hw,pBE,ci,stats] = ttest(BERr1,BERc1)
     [hrd,pAE,ci,stats] = ttest(BERr2,BERc2)
...
main_handle = figure; hold on;
        ...
        iBSr = interp1((FqBSRa/10)/scaleDown,xi,'spline');
        iBTr = interp1((FqBTRa/10)/scaleDown,xi,'spline');
        iACr = interp1((FqACRa/10)/scaleDown,xi,'spline');
        iBEr = interp1((FqBERa/10)/scaleDown,xi,'spline');
        iAEr = interp1((FqAERa/10)/scaleDown,xi,'spline');
       h1= plot(XX1, iBSr,'k');
       h2= plot(XX1, iBTr,'c');
       h3= plot(XX1, iACr,'g');
       h4= plot(XX1, iBEr,'r');
...
[h_main, h_inset]=inset(main_handle, inset_handle, inset_size)
       ...
       main_handle = figure; hold on;
        ...
        xi = 1:0.25:25; 
        XX1 = [1 :0.5 :length(FqDis)*2-1];
        iBSr = interp1((FqBSWa/10)/scaleDown,xi,'spline');
        iBTr = interp1((FqBTWa/10)/scaleDown,xi,'spline');
        iACr = interp1((FqACWa/10)/scaleDown,xi,'spline');
        iBEr = interp1((FqBEWa/10)/scaleDown,xi,'spline');
        iAEr = interp1((FqAEWa/10)/scaleDown,xi,'spline');
       h1= plot(XX1, iBSr,'k');
       h2= plot(XX1, iBTr,'c');
       h3= plot(XX1, iACr,'g');
       h4= plot(XX1, iBEr,'r');
...
%     tmp =1;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%AC
%[hbe,pbe,ci,stats] = ttest(AC3(:,1),AC3(:,2))
h = figure;
hold on;
%AC3 = AC./[mean([BS BT BE AE],2) mean([BS BT BE AE],2)];
AC2 = AC./[mean([AC],2) mean([AC],2)];
AC3 = AC2- mean(mean(AC))*0.37/scaleDown;
bar(AC3);
legend('SL','FD');
    titleName = ['Theta power after cue on'];
 %   title(titleName,'FontSize',16);
