...
BSBE123(1,2) = mean([BERc1 BERr1]);
BSBE123(2,2) = mean([BEWc1 BEWr1]);
BSBE123(1,3) = mean([BERc2 BERr2]);
BSBE123(2,3) = mean([BEWc2 BEWr2]);
BSBE123(1,4) = mean([BERc3 BERr3]);
BSBE123(2,4) = mean([BEWc3 BEWr3]);
 [hr,pr,ci,stats] = ttest(BEWc2+BEWr2,BEWc3+BEWr3)
        h = figure; hold on;
        bar(BSBE123);
