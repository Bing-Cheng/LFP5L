...
for i = 1 : ratNumber
    ...
    wBE = [25:125];
    load([odir rat 'fftBEcr']);%, 'fftR1Dmc', 'fftW1Dmc');
    FqBERr = mean(fftR1DmrBE(FqDis,wBE),2);
    FqBEWr = mean(fftW1DmrBE(FqDis,wBE),2);  
    FqBERc = mean(fftR1DmcBE(FqDis,wBE),2);
    FqBEWc = mean(fftW1DmcBE(FqDis,wBE),2);  
    FqBER = (FqBERr+FqBERc)/2;
    FqBEW = (FqBEWr+FqBEWc)/2;
...
%         saveas(h,[odir titleN],'jpg');
        ...
        BE(i,1) = mean(FqBER(thetaBand));
        BE(i,2) = mean(FqBEW(thetaBand));
        BERc(i) = mean(FqBERc(thetaBand));
        BERr(i) = mean(FqBERr(thetaBand));
        BEWc(i) = mean(FqBEWc(thetaBand));
        BEWr(i) = mean(FqBEWr(thetaBand));
  ...
  % plot(x2(2),yy(2),x2(3),yy(3),x2(4),yy(4),x2(1),yy(1),'color',[0.9 0.8 0.8]);
       ...
       h1= plot(XX1, interp1(XX,FqBSRa,xi,'spline'),'g');
       h2 = plot(XX1, interp1(XX,FqBSWa,xi,'spline'),'--g');
       h3 = plot(XX1, interp1(XX,FqBTRa,xi,'spline'),'r');
       h4 =  plot(XX1, interp1(XX,FqBTWa,xi,'spline'),'--r');
       h5 =  plot(XX1, interp1(XX,FqACRa,xi,'spline'),'b');
       h6 =  plot(XX1, interp1(XX,FqACWa,xi,'spline'),'--b');
       h7 =  plot(XX1, interp1(XX,FqBERa,xi,'spline'),'m');
       h8 =  plot(XX1, interp1(XX,FqBEWa,xi,'spline'),'--m');

        legend([h1 h2 h3 h4 h5 h6 h7 h8],'BS-SL','BS-FD','BT-SL','BT-FD','AC-SL','AC-FD','BE-SL','BE-FD');
        xlabel('Frequency (Hz)');
