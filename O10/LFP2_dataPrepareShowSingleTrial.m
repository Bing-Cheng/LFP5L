...
dir = ['H:\sectionedDataLFP\O10\sectioned\'];
odir =  'H:\LFP5LOutput\O10\trialWave\';
windowLength = 6000;
fftLength = 500;
fs= 24414;
rfs = 1000;
...
for i = 1:trialNumberCorrect
    ...
    if k>7
        ...
        titleName = ['Correct-' int2str(trialNumberCorrect) rat '-' date '-ch' int2str(chI) '-' int2str(fig) '-' int2str(k)];
            title(titleName);
...
for i = 1:trialNumberIncorrect
    ...
    if k>7
        ...
        titleName = ['Incorrect-' int2str(trialNumberIncorrect) rat '-' date '-ch' int2str(chI) '-' int2str(fig) '-' int2str(k)];
            title(titleName);
            saveas(h,[odir titleName],'jpg');
         k=1;
         fig = fig +1;
